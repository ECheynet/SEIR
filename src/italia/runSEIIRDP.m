...
% do the fit
[alpha_fit, beta_fit, gamma_fit, delta_fit, lambda_fit, kappa_fit, tau_fit, rho_fit] = fit(TotPositive, Recovered, Deaths, Npop, E0, Ia0, time, guess, 'Display', 'off');

%% apply model with fitted parameters
[S, E, Ia, Iq, R, D, P] = model(alpha_fit, beta_fit, gamma_fit, delta_fit, lambda_fit, kappa_fit, tau_fit, rho_fit, Npop, E0, Ia0, Iq0, R0, D0, t);
...
semilogy(time1, Ia, 'blue', time1, Iq + R + D, 'cyan', time1, Iq, 'red', time1, R, 'green', time1, D, 'black');  % simulation
hold on
...
xlabel('time (days)')
title('Italy');
leg = {'line 76 ...', 'asymptomatic', 'total = positives + recovered + dead', 'positives = quarantined + hospitalized', 'recovered', 'dead'};
